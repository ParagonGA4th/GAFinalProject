...
for %%A in ("%target_directory%") do (
    powershell.exe -Command "Get-ChildItem -Path '%%~A' -Filter *.cpp -Recurse | Where-Object {$_.Name.Substring(0,6) -eq 'unity_'} | Remove-Item -Force"
    powershell.exe -Command "Get-ChildItem -Path '%%~A' -Filter *.obj -Recurse | Where-Object {$_.Name.Substring(0,6) -eq 'unity_'} | Remove-Item -Force"
)
